...
// String method to customize the string representation of the Block
func (b *Block) String() string {
	// Format the byte slice as a hex string for readability
	bbStr := fmt.Sprintf("bb: %x", b.bb) // Convert []byte to hex string

	// Format the map as a string
	bmStr := fmt.Sprintf("bm: %v", b.bm) // Format the map as a string (can be customized)

	// Print the logger info as a string (just the logger's string representation)
	logStr := fmt.Sprintf("Logger: %v", b.log)

	// Combine all fields into a readable format
	return fmt.Sprintf("Block{bb: %s, bm: %s, op: %v, %s}", bbStr, bmStr, b.op, logStr)
}
