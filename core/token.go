...
func (c *Core) syncTokenChain(req *ensweb.Request) *ensweb.Result {
	...
	log.Println("syncTokenChain next block id is ", nextID)
...
func (c *Core) syncTokenChainFrom(p *ipfsport.Peer, pblkID string, token string, tokenType int) error {
	...
	if blk != nil {
		...
		log.Println("blockid for sync is ", blkID)
		if blkID == pblkID {
			return nil
		}
		blockNumber, err := blk.GetBlockNumber(token)
		if err != nil {
			c.log.Error("Failed to get blocknumber, ", "err : ", err)
		}
		log.Println("Latest token block of ", token, " is ", blockNumber)
	...
	for {
		...
		if err != nil {
			c.log.Error("Failed to sync token chain block: sendJSONRequest ", "err", err)
			return err
		}
		if !trep.Status {
			c.log.Error("Failed to sync token chain block : !trep.Status ", "msg", trep.Message)
