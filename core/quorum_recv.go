...
func (c *Core) updateReceiverToken(req *ensweb.Request) *ensweb.Result {
	...
	log.Println("Token chain block received in byte is ", sr.TokenChainBlock)
	...
	for _, ti := range sr.TokenInfo {
		...
		log.Println("Previous blockid got ", pblkID)
		log.Println("initating TC sync from ", p)
